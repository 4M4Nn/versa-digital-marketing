...
export default function NumbersSection() {
  ...
  return (
    <section ref={sec} className="relative min-h-screen flex items-center justify-center bg-[#0A1518] overflow-hidden">
      <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 40% 40% at 50% 50%, rgba(0,201,184,0.05) 0%, transparent 70%)" }} />
      <div className="relative z-10 text-center px-6">
        {numberPhases.map((p, i) => (
          <div key={i} className="n-phase" style={{ position: i === 0 ? "relative" : "absolute", inset: i === 0 ? "auto" : 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
            ...
            <p className="label text-[#6B8080] mt-4" style={{ letterSpacing: "3px" }}>{p.label}</p>
          </div>
        ))}
      </div>
    </section>
